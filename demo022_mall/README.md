# demo022_mall - 小型电商系统演进版（Phase 4：订单系统基础闭环已完成）
...
`demo022_mall` 是整个教学系列的**订单系统演进版本**：

- **demo001 ~ demo020**：以「学生管理系统」为载体，系统性地学习 JDBC → MyBatis → Spring Boot → 生产级基础设施（日志、异常、事务、Docker、JWT 等）。
- **demo022_mall** 实现了 **Phase 4：订单系统基础闭环**。后续版本将继续围绕「用户 → 商品 → 购物车 → 订单 → 支付 → 优化 → 工程化」演进。

**本版本（demo022_mall）** 已完成 **Phase 4：订单系统基础闭环**（下单事务、库存扣减、购物车清空 + 查询）。
...
准备好进入下一个小步骤了吗？随时告诉我——我们是继续优化 demo022_mall，还是直接开始 demo023_mall 的订单状态机？