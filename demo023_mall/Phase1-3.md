...
> **历史基线文档说明**  
> 本文档完整记录了 **demo021_mall** 的 Phase 1-3 演进过程。  
> **demo023_mall（及后续版本）完整继承 demo021_mall 的 Phase 1-3 基线**，包括用户认证、商品域、购物车等核心能力。  
> 本文档内容保持原样，仅作为历史参考。当前项目的“当前状态”描述均指 demo021_mall 当时的情况。

> 本文档记录了 demo021_mall 从「demo020 成熟骨架」到当时状态所经历的三个主要阶段。
...
### demo021_mall 当时定位（历史）

- **已完成**：Phase 1（认证）、Phase 2（商品）、Phase 3（购物车）
- **核心价值**：拥有了一个可以真实「注册-登录-浏览商品-加购物车」的小型电商后端骨架。
- **下一步（历史规划）**：demo022_mall 将在此基础上添加**订单系统**（最有教学价值的交易闭环）。

> **注意**：以上“当前/已完成”描述均为 demo021_mall 历史阶段的记录。demo023_mall 已在此基础上完成了 Phase 4（订单基础闭环）和 Phase 5（状态机 + 取消订单）。
