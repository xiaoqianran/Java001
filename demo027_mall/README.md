# demo027_mall - 小型电商系统演进版（Phase 9：模拟第三方支付回调与支付幂等）
...
> **当前状态**：Phase 9 已完成 —— 模拟第三方支付回调与支付幂等（通过支付单 + mock-callback 驱动订单 10 → 20）。继承 demo026_mall 的超时自动取消能力。

---

## 项目定位

`demo027_mall` 是整个教学系列的**支付回调与幂等演进版本**：
...
**本版本（demo027_mall）** 已完成 **Phase 9：模拟第三方支付回调与支付幂等**。
...
- 新增接口：
  - `POST /api/payment/order/{orderId}` —— 创建模拟支付单
  - `POST /api/payment/mock-callback` —— 模拟第三方支付回调（驱动订单 10 → 20）
- 强调支付幂等性、回调事务边界、与订单超时自动取消的并发安全
- 推荐新的支付流程（替代 Phase 6 的直接 /pay 接口）
