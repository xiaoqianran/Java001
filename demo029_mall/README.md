...
> **当前状态**：Phase 11 已完成 —— 退款申请与审核流程（买家提交退款申请 → 管理员/卖家审核 → 审核通过后执行实际退款、订单状态 20→60、支付单 20→40 + 库存恢复）。强调工作流、审批流、申请与执行分离。
...
- **前一版本 (Phase 10 基线)**：模拟退款流程：已支付未发货 20 → 60 已退款 + 支付单退款 + 库存恢复。
...
- [Phase11.md](./Phase11.md)（退款申请与审核流程 - 本阶段已完成）
- [API-接口文档.md](./API-接口文档.md)

**demo029_mall Phase 11 退款申请与审核流程已完成（基于 Phase 10 完整基线）。**
