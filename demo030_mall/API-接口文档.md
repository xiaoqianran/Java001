# demo030_mall API 接口文档（Phase 12 - 部分退款与退款金额校验）

> **项目**：demo030_mall - 小型电商系统演进版（Phase 12：部分退款与退款金额校验）  
> **当前版本**：Phase 1-12 已完成（用户认证 + 商品域 + 购物车 + 订单基础 + 状态机/取消 + 支付模拟 + 发货完成 + 超时自动取消 + 支付回调幂等 + 模拟退款 + 退款申请审核 + 部分退款金额校验）  
...
## 13. 退款申请与审核接口（Phase 11 基线 + Phase 12 金额支持）
...
- 校验申请 status=10
- 调用退款执行：使用 refund_order.refund_amount 调用 PaymentService.refundOrder(operator, orderId, refundAmount)
- 更新 payment_order.refunded_amount
- 更新申请 status=20 + 审核信息
- 即使是部分金额退款，本阶段订单仍进入 60 终态（教学简化）
...
完整设计见 [Phase11.md](./Phase11.md) 和 [Phase12.md](./Phase12.md)。

---

**文档生成时间**：2026-05-19  
**验证状态**：demo030_mall Phase 12 部分退款与退款金额校验已完成，mvn package 通过，文档已自洽。
本阶段在 Phase 11 基础上新增 refund_amount / refunded_amount 字段 + 金额校验 + 按指定金额退款。