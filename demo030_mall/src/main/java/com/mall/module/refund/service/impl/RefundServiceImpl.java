...
public class RefundServiceImpl implements RefundService {
    ...
    public Long applyRefund(LoginUser buyer, Long orderId, RefundApplyDTO dto) {
        ...
        // 2. 查询支付单并校验退款金额（Phase 12 核心新增）
        PaymentOrder payment = paymentOrderMapper.selectOne(
                new LambdaQueryWrapper<PaymentOrder>()
                        .eq(PaymentOrder::getOrderId, orderId)
        );
        if (payment == null || payment.getStatus() != 20) {
            throw new BusinessException("订单支付单状态异常，无法申请退款");
        }

        java.math.BigDecimal refundAmount = dto.getRefundAmount();
        ...
        // 3. 校验是否已有任何退款申请（本阶段一订单只允许一条记录，拒绝后不重复申请；用唯一约束防并发）
